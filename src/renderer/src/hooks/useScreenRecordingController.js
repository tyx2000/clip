...
export function useScreenRecordingController({
   ...
   */
  async function beginRecordingWithSource(sourceId, cloudSyncEnabled) {
    if (!sourceId) {
   ...
   */
  async function handleConfirmSourceAndStart() {
   ...
   */
  async function handleOpenRecording(path) {
   ...
   * @param {string} path 目标录屏文件路径。
   */
  async function handleRevealRecording(path) {
   ...
   */
  async function handleDeleteRecording(item) {
   ...
   * @param {object} item 当前录屏卡片对应的数据项。
   */
  async function handleRetryCloudSync(item) {
    if (typeof window.api?.retryCloudSyncSession !== 'function') {
      setStatusMessage('Cloud sync retry API unavailable. Please restart the Electron app process.')
      return
    }
   ...
   * @param {object} item 当前录屏卡片对应的数据项。
   */
  async function handleDeleteRecordingWithGuard(item) {
    /** 尚未上传完成的分片数量。 */
